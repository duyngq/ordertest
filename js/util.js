...
function calFeeAmount(parentId, weight, price, unit, unitPrice, total) {
  var weight = parseFloat(
    document.getElementById(parentId).getElementsByClassName(weight)[0].value
  ); // addClient.lender_loanAmount.value;
  var pricePerWeight = parseFloat(
    document.getElementById(parentId).getElementsByClassName(price)[0].value
  ); // addClient.builder_packageAmount.value;
  var unit = parseFloat(
    document.getElementById(parentId).getElementsByClassName(unit)[0].value
  ); // addClient.lender_loanAmount.value;
  var unitPrice = parseFloat(
    document.getElementById(parentId).getElementsByClassName(unitPrice)[0].value
  ); // addClient.lender_loanAmount.value;
  var totalPricePackage = (weight * pricePerWeight + unit * unitPrice).toFixed(
    4
  );
  document
    .getElementById(parentId)
    .getElementsByClassName(total)[0].value = totalPricePackage;
}
function calTotal(parentId) {
  ...
  for (i = 0; i < 11; i++) {
    totalId = "total" + i;
    total += parseFloat(
      document.getElementById(parentId).getElementsByClassName(totalId)[0].value
    );
  }
  document
    .getElementById(parentId)
    .getElementsByClassName("prm_sum")[0].value = (total +
    parseFloat(
      document.getElementById(parentId).getElementsByClassName("add_fee")[0]
        .value
    )
  ).toFixed(4);
}
...
function calTotalWeight(parentId) {
  ...
  for (i = 0; i < 11; i++) {
    weight = "weight" + i;
    weightSum += parseFloat(
      document.getElementById(parentId).getElementsByClassName(weight)[0].value
    );
  }
  document
    .getElementById(parentId)
    .getElementsByClassName("weight_sum")[0].value = weightSum;
}
...
function calTotalUnit(parentId) {
  ...
  document
    .getElementById(parentId)
    .getElementsByClassName("unit_sum")[0].value = unitSum;
}
...
function updateTotal(tableId) {
  ...
  for (var r = 1, n = table.rows.length; r < n; r++) {
    total =
      parseFloat(total) + parseFloat(table.rows[r].cells[3].children[0].value);
  }
  document.getElementById("prm_sum").value =
    parseFloat(total) +
    parseFloat(document.getElementById("total_package_price").value);
}

function updateTotal() {
  document.getElementById("prm_sum").value =
    parseFloat(document.getElementById("total_package_price").value) +
    parseFloat(document.getElementById("total_package_price_1").value) +
    parseFloat(document.getElementById("total_package_price_2").value) +
    parseFloat(document.getElementById("add_fee").value);
}
...
function addProductRow(tableId) {
  ...
  table.rows[tableRows].cells[0].innerHTML =
    '<td><input name="product' +
    e +
    'name" type="text" id="product' +
    e +
    'name" size="30" placeholder="Product name" /></td>';

  table.rows[tableRows].insertCell(1);
  table.rows[tableRows].cells[1].innerHTML =
    '<td><input name="product' +
    e +
    'quantity" type="number" id="product' +
    e +
    'quantity" value="0" size="30" onchange="calProductAmount(\'product' +
    e +
    "quantity', 'product" +
    e +
    "price', 'product" +
  ...
  table.rows[tableRows].cells[2].innerHTML =
    '<td><input name="product' +
    e +
    'price" type="text" id="product' +
    e +
    'price" value="0" size="30" onchange="calProductAmount(\'product' +
    e +
    "quantity', 'product" +
    e +
    "price', 'product" +
  ...
  table.rows[tableRows].cells[3].innerHTML =
    '<td><input name="product' +
    e +
    'amount" type="text" id="product' +
    e +
    'amount" value="0" size="30" readonly="true"/></td>';

  table.rows[tableRows].insertCell(4);
  table.rows[tableRows].cells[4].innerHTML =
    '<td><input type="button" onclick="addProductRow(\'productTbl\')" border=0 style=\'cursor:hand\' value="+"/></td>';

  table.rows[tableRows].insertCell(5);
  table.rows[tableRows].cells[5].innerHTML =
    '<td><input type="button" onclick="removeProductRow(\'productTbl\', this)" border=0 style=\'cursor:hand\' value="-"/></td>';
...
function openProductDescWindow() {
  ...
  childWindow = window.open(
    "productdesc.html",
    "win2",
    "status=no,toolbar=no,scrollbars=yes,titlebar=no,menubar=no,resizable=yes,directories=no,location=no,top=" +
      top +
      ", left=" +
      left
  );
...
function openFeeWindow() {
  ...
  childWindow = window.open(
    "fee.html",
    "win2",
    "status=no,toolbar=no,scrollbars=yes,titlebar=no,menubar=no,resizable=yes,width=800,height=400,directories=no,location=no,top=" +
      top +
      ", left=" +
      left
  );
