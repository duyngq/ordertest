...
if (isset($_POST["sbmAddComment"])) {
    $sbmAddComment = $_POST["sbmAddComment"];
}
if (isset($sbmUpdateInfo) || isset($sbmAddComment)) {
    $custId = $_POST["custId"];
    ...
    if (isset($sbmUpdateInfo)) {
        ...
        $custPhone = $_POST["referrer_name"];
        $totalWeight = $_POST["referrer_fee"];
        if (!is_null($totalWeight)) {
            if (!is_numeric($totalWeight)) {
        ...
        $prm_financial_broker_status = $_POST['ckb_financialBroker'] == 'on' ? 1 : 0;
        $prm_legals_status = $_POST['ckb_legals'] == 'on' ? 1 : 0;
        $prm_builders_status = $_POST['ckb_builders'] == 'on' ? 1 : 0;
        $prm_financial_planner_status = $_POST['ckb_financialPlanner'] == 'on' ? 1 : 0;
        
        $newCustomerArray = array("referrer_name" => $custPhone, "referrer_fee" => $totalWeight,
                    ...
                    "prm_commission" => $prmCommission, "prm_financial_broker" => $prmFinancialBroker, "prm_legals" => $prmLegals, "prm_builders" => $prmBuilders, "prm_financial_planner" => $prmFinancialPlanner, "prm_total" => $prmTotal,
                    "status" => $status, "prm_financial_broker_status" => $prm_financial_broker_status, "prm_legals_status" => $prm_legals_status, "prm_builders_status" => $prm_builders_status, "prm_financial_planner_status" => $prm_financial_planner_status);
        ...
        $customerInfoArray = array("referrer_name" => "[Referrer/Name", "referrer_fee" => "[Referrer/Fee]",
                    ...
                    "prm_commission" => "[PRM Commissions Receivable]", "prm_financial_broker" => "[PRM Commissions/Financial Broker]", "prm_legals" => "[PRM Commissions/Legals]", "prm_builders" => "[PRM Commissions/Builders]", "prm_financial_planner" => "[PRM Commissions/Financial Planner]", "prm_total" => "[PRM Commissions/Total]",
                    "status" => "[Status]", "prm_financial_broker_status" => "[PRM Commissions/Financial Broker]", "prm_legals_status" => "[PRM Commissions/Legals]", "prm_builders_status" => "[PRM Commissions/Builders]", "prm_financial_planner_status" => "[PRM Commissions/Financial Planner]");
        foreach ($compareNewCustAndOldCust as $key => $value) {
            ...
            if (!is_null($newValue) || !empty($newValue) || isset($newValue)) {
                ...
                if ($key == 'prm_financial_broker_status' || $key == 'prm_legals_status' || $key == 'prm_builders_status' || $key == 'prm_financial_planner_status') {
                    if ($newValue == 1) {
                        $systemLog = $systemLog."<em><span style='color:#FF0000'>*System comment:</span> <strong>".$customerInfoArray[$key]."</strong> changed from <strong>No</strong> to <strong>Yes</strong>.. </em>";
                    } else {
                        $systemLog = $systemLog."<em><span style='color:#FF0000'>*System comment:</span> <strong>".$customerInfoArray[$key]."</strong> changed from <strong>Yes</strong> to <strong>No</strong>.. </em>";
                    }
                } else {
    ...
    //add comment
    if (isset($sbmAddComment)) {
        unset($_POST['sbmAddComment']);
...
//        }
        if ($addCommentQuery != "INSERT INTO comments(date, comment, cust_id, user_name) VALUES") {
            ...
            if ($addCommentResult) {
                $updateDateForCustomerQuery = "UPDATE customers SET date = '".$currentDate."', user_name='".$username."' WHERE id=$custId";
                $updatedateForCustomerResult = mysql_query($updateDateForCustomerQuery, $connection) or die(mysql_error() . "Can not store customers to database");
                if ($updatedateForCustomerResult) {
    ...
    <td><div align="right">
      ...
      <table width="100%" border="0" bordercolor="#F0F0F0">
          ...
          while ($customer = mysql_fetch_array($customerResult)) {
              $userId = $_SESSION['user_id'];
              $getUserOfCustQuery = "SELECT * FROM customers WHERE id = $custId";
              ...
              $oldCustArray = array("referrer_name" => $customer['referrer_name'], "referrer_fee" => $customer['referrer_fee'],
                  ...
                  "prm_commission" => $customer['prm_commission'], "prm_financial_broker" => $customer['prm_financial_broker'], "prm_legals" => $customer['prm_legals'], "prm_builders" => $customer['prm_builders'], "prm_financial_planner" => $customer['prm_financial_planner'], "prm_total" => $customer['prm_total'],
                  "status" => $status, "prm_financial_broker_status" => $customer['prm_financial_broker_status'], "prm_legals_status" => $customer['prm_legals_status'], "prm_builders_status" => $customer['prm_builders_status'], "prm_financial_planner_status" => $customer['prm_financial_planner_status']);
        ...
        <tr style="border-bottom:1px solid">
          <td colspan="6" style="border-bottom:1px solid">- <strong>Status</strong>: 
            <select name="status" <?php if ($_SESSION['username'] != 'admin') { echo 'disabled';} ?>>
              <option value="1" <?php if ($_SESSION['status'] == 1) { echo "selected='selected'";}?>>Activate</option>
              <option value="0" <?php if ($_SESSION['status'] == 0) { echo "selected='selected'";}?>>Deactivate</option>
          </select></td>
        </tr>
        <tr>
      ...
      <!--    <td width="44%">&nbsp;</td>-->
        ...
        <tr>
          <td colspan="3" style="border-bottom:1px solid"> - <strong>PRM Commission Receivable</strong>:
          <input type="text" name="prm_commission" id="prm_commission" value="<?php echo $customer['prm_commission'];?>" <?php if ($status == 0) { echo 'disabled'; } ?> /></td>
          <td style="border-bottom:1px solid"><strong>Received</strong>:</td>
        ...
        <tr>
		  ...
		  <td><input type=checkbox NAME="ckb_financialBroker" 
                      <?php
                        if ($customer['prm_financial_broker_status'] > 0) {
                            echo 'checked';
                        }
                      ?>
                      >
        ...
        <tr>
		  ...
		  <td><input type=checkbox NAME="ckb_legals" 
                      <?php 
                        if ($customer['prm_legals_status'] > 0) {
                            echo 'checked';
                        }
                      ?>
                      >
        ...
        <tr>
		  ...
		  <td><input type=checkbox NAME="ckb_builders"
                      <?php 
                        if ($customer['prm_builders_status'] > 0) {
                            echo 'checked';
                        }
                      ?>
                      >
        ...
        <tr>
            <td style="border-bottom:1px solid"></td>
            <td style="border-bottom:1px solid">Financial Planner: </td>
            ...
            <td style="border-bottom:1px solid"><input type=checkbox NAME="ckb_financialPlanner"
                                                <?php 
                                                    if ($customer['prm_financial_planner_status'] > 0) {
                                                        echo 'checked';
                                                    }
                                                ?>
                                                 >
            </td>
            <td colspan="2" style="border-bottom:1px solid">Total: $<input name="prm_sum" type="text" id="prm_sum" value="<?php echo $customer['prm_total']; ?>" size="5"  <?php if ($status == 0) { echo 'disabled'; } ?> readonly="true"/></td>
        </tr>
          <?php } ?>
      </table>
      <div align="center">
<form id="frmUpdateInfo" name="frmUpdateInfo" method="post" style="text-align:center">
            <p>
                <input type="submit" name="sbmUpdateInfo" value="Update Information" onclick="return confirm('Are you sure you want to change ?')" />
            </p>
</form>
      </div>
      <table width="100%" border="0">
        <tr>
		  ...
		  <p>
                      <textarea name="comment" cols="150" rows="10" style="border:2px solid #ff0000"></textarea>
          </p>
          <p align="center">
            <input name="sbmAddComment" type="submit" id="sbmAddComment" value="Submit Comment" />
</p></td>
        </tr>
      </table>
